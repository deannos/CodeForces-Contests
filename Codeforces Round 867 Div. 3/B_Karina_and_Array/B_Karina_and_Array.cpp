...
void solve()
{
    ...
    srt(vec);
    //find the smallest and second element product with largest and second largest element
    //get maximum product as required answer
    int ans = 0;
    fo(i, n)
    {
        ans = max((vec[0] * vec[1]), (vec[n-1] * vec[n-2]));
    }
    cout << ans << endl;
